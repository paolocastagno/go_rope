...
for i in $(seq $begin $end); do
	...
	awk 'NR==FNR{ts[$6]=$7;cid[$6]=$5;next}{print (ts[$6]? $5 " " cid[$6] " " ($7-ts[$6]) : "missing")}' ${dir}/${fnm_u}_${i} ${dir}/${fnm_d}_${i} > ${dir}/${fnm_o}_${i}.csv
	
	sed "${OPT[@]}" '/missing/d' ${dir}/${fnm_o}_${i}.csv
	latency_1[$((i-1))]=$(awk '{($1==$2)? latency+=$3 : latency+=0; ($1==$2)? cnt++ : cnt+=0  }END{print (cnt!=0)? (1e-6*latency)/cnt : cnt}' ${dir}/${fnm_o}_${i}.csv)
	latency_2[$((i-1))]=$(awk '{($1!=$2)? latency+=$3 : latency+=0; ($1==$2)? cnt++ : cnt+=0 }END{print (cnt!=0)? (1e-6*latency)/cnt : cnt}' ${dir}/${fnm_o}_${i}.csv)
